...
(function() {
    ...
    function createPirateShip(config) {
        ...
        // Hull - dark wood
        const hullGeometry = getGeometry('box', 8, 3, 20);
        const hullMaterial = getMaterial('lambert', { color: 0x4a3020 });
        ...
        // Deck
        const deckGeometry = getGeometry('box', 7, 0.5, 18);
        const deckMaterial = getMaterial('lambert', { color: 0x6a5040 });
        const deck = new THREE.Mesh(deckGeometry, deckMaterial);
        deck.position.y = 3;
        shipGroup.add(deck);

        // Mast
        const mastGeometry = getGeometry('cylinder', 0.3, 0.4, 12, 8);
        const mastMaterial = getMaterial('lambert', { color: 0x3a2510 });
        const mast = new THREE.Mesh(mastGeometry, mastMaterial);
        mast.position.y = 9;
        shipGroup.add(mast);

        // Sail
        const sailGeometry = getGeometry('plane', 6, 8);
        ...
        const skullGeometry = getGeometry('circle', 1.5, 8);
        const skullMaterial = getMaterial('basic', { color: 0xffffff });
        ...
        // Cannons (sides)
        for (let i = 0; i < 3; i++) {
            const cannonGeometry = getGeometry('cylinder', 0.3, 0.4, 2, 8);
            const cannonMaterial = getMaterial('lambert', { color: 0x222222 });
